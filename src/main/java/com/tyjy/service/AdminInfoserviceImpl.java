...
public class AdminInfoserviceImpl implements AdminInfoservice {
    ...
    public String Login(AdminInfo adminInfo, HttpServletRequest request) {
        ...
        if (admin == null) {
            ...
            if (pwd.equals(admin.getAdminPwd())) {
                if (redisUtil.hasKey(admin.getAdminName() + "," + admin.getAdminPwd())) {
                    Object object = redisUtil.get(admin.getAdminName() + "," + admin.getAdminPwd());
                ...
                if (redisUtil.hasKey(admin.getAdminName() + "," + admin.getAdminPwd())) {
                    ...
                    int num = Integer.parseInt(object.toString());
                    if (num < 3) {
                        redisUtil.incr(admin.getAdminName() + "," + admin.getAdminPwd(), 1);
                        redisUtil.expire(admin.getAdminName() + "," + admin.getAdminPwd(), 60);
                        num = 3 - num;
                    ...
                    redisUtil.set(admin.getAdminName() + "," + admin.getAdminPwd(), 1, 60);
                    return "密码错误,还有3次机会";
                }
            }
        }
    ...
    @Override
    public String del(AdminInfo adminInfo, HttpServletRequest request) {
        HttpSession session =request.getSession();
        AdminInfo admin= (AdminInfo) session.getAttribute("admin");
        session.setAttribute("admin",admin);
        adminInfo=adminInfoDao.selectById(adminInfo);
        admin=adminInfoDao.selectById(admin);
        if(adminInfo.getAdminId().equals(admin.getAdminId())){
            return "不能删除自己";
        }
        else {
            if(admin.getLeader().equals("是")){
                int num=adminInfoDao.del(adminInfo);
                if(num>0){
                    return "删除成功";
                }
            }
